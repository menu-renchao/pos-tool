...
class WindowsService(Backend):
    ...
    def scan_local(self, base_path, env):
        ...
        self.log(f"正在扫描 {base_path} 中的版本目录...")
        need_modify_files = []
        ...
        for item in os.listdir(base_path):
            ...
            for i, pattern in enumerate(self.file_patterns):
                ...
                if os.path.isfile(file_path):
                    try:
                        ...
                        self.log(f"读取文件出错: {file_path} - {str(e)}")
            ...
            if os.path.isfile(app_prop_path):
                try:
                    ...
                    self.log(f"读取文件出错: {app_prop_path} - {str(e)}")

        if not need_modify_files:
            self.log("未找到需要修改的文件")
        else:
            self.log("需要修改的文件路径如下：")
    ...
    def _modify_local_app_properties(self, version_path, env):
        ...
        try:
            ...
            self.log(f"处理 application.properties 出错: {str(e)}")
            return False, False
    ...
    def replace_war_windows(self, base_path, selected_version, local_war_path):
        ...
        try:
            ...
            if os.path.exists(war_path):
                self.log("正在删除旧的 kpos.war ...")
                os.remove(war_path)
            if os.path.exists(kpos_path):
                self.log("正在删除旧的 kpos 目录 ...")
                shutil.rmtree(kpos_path)

            # 复制新 WAR 包
            self.log("正在复制新的 kpos.war ...")
            shutil.copy(local_war_path, war_path)

            # 解压新 WAR 包
            self.log("正在解压新的 kpos.war ...")
            with zipfile.ZipFile(war_path, 'r') as zip_ref:
                zip_ref.extractall(kpos_path)
            self.log("解压完成")
            self.log("如需要修改环境，请使用修改功能，然后重启POS；如不需要修改环境，请直接重启POS。")
            self.log(f"{selected_version} 替换 kpos.war 成功！")
        except Exception as e:
            self.log(f"替换 kpos.war 出错: {str(e)}")
    ...
    def restart_pos_windows(self, base_path, selected_version):
        ...
        try:
            ...
            self.log(f"{selected_version} POS 重启成功！")
        except Exception as e:
