...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.progress_timer = QTimer(self)  # 初始化定时器，防止AttributeError
        self.setup_backend()
        self.setup_ui()
    ...
    def create_tab_contents(self):
        ...
        try:
            ...
            self.append_log(f"❌ Linux POS模块加载失败: {str(e)}", "error")
        ...
        try:
            ...
            self.append_log(f"❌ Windows POS模块加载失败: {str(e)}", "error")
        ...
        try:
            ...
            self.append_log(f"❌ Caller ID模块加载失败: {str(e)}", "error")
        ...
        try:
            ...
            self.append_log(f"❌ License Backup模块加载失败: {str(e)}", "error")
        ...
        try:
            ...
            self.append_log(f"❌ Download War模块加载失败: {str(e)}", "error")
        ...
        try:
            ...
            self.append_log(f"❌ 图片生成模块加载失败: {str(e)}", "error")
...
QPushButton:disabled {
    ...
    def append_log(self, msg: str, level: str = "info"):
        ...
        self.log_text.append_colored_text(formatted_msg, color)
    ...
    def setup_progress_animation(self, duration: int = 1000):
        """设置进度条动画和递增进度"""
        self.progress_bar.setVisible(True)
        self.fake_progress = 0
        self.progress_bar.setValue(0)
        self.progress_timer.start(max(10, duration // 100))  # 100步渐变，最小10ms一跳
